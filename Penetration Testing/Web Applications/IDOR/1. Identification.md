...
URL or API parameters that reference an object like so e.g. `?uid=1` or `?filename=file_1.pdf` may be vulnerable
...
##### Base64
if we see a reference like (`?filename=ZmlsZV8xMjMucGRm`), we can immediately guess that the file name is `base64` encoded (from its character set), which we can decode to get the original object reference of (`file_123.pdf`)
##### Hash
